...
export function SequenceCard({
  ...
  onVolumeChange,
  onReverbChange,
  // Sample-specific props
  playbackRate,
  loop,
  startTime,
  endTime,
  onSampleSettingChange
}) {
  ...
  return (
    ...
    >
      ...
      {/* Mobile long-press menu modal - rendered outside card using portal */}
      {showMenu && createPortal(
        <>
          <div className="card-menu-backdrop" onClick={() => setShowMenu(false)} />
          <div className="card-menu-modal" ref={menuRef} onClick={(e) => e.stopPropagation()}>
            <div className="card-menu-header">
              <h3 style={{ color: sequence.color }}>{sequence.name}</h3>
              <button
            ...
            <div className="card-menu-content">
              ...
              <div className="menu-sliders">
                <div className="menu-slider">
                  ...
                  <input
                    ...
                    style={{ '--slider-color': sequence.color }}
                  />
                </div>
                {!sequence.isSample && (
                  <div className="menu-slider">
                ...
                {sequence.isSample && (
                  <>
                    <div className="menu-slider">
                      <label>
                        <span>Speed</span>
                        <span className="value">{playbackRate?.toFixed(2)}x</span>
                      </label>
                      <input
                        type="range"
                        min="0.25"
                        ...
                        onChange={(e) => {
                          e.stopPropagation();
                          onSampleSettingChange?.(sequence.id, 'playbackRate', Number(e.target.value));
                        }}
                        onClick={(e) => e.stopPropagation()}
                        style={{ '--slider-color': sequence.color }}
                      />
                      ...
                      <label>
                        ...
                        <label className="toggle-switch">
                          <input
                            ...
                            onChange={(e) => {
                              e.stopPropagation();
                              onSampleSettingChange?.(sequence.id, 'loop', e.target.checked);
                            }}
                            onClick={(e) => e.stopPropagation()}
                          />
              ...
              <div className="menu-actions">
                {/* Edit button - only for non-sample sequences */}
                {!sequence.isSample && (
                ...
                {/* Copy button - only for non-sample sequences */}
                {!sequence.isSample && (
